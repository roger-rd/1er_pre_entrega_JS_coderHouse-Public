...
const platos = {
  ...
  const obtenerOpcionPorID = (tipoPlato) => {
    ...
    do {
      opcion = prompt(
        `Ingrese el número de la opción de ${tipoPlato}:\n${obtenerOpcionesPorID(
          platos[tipoPlato]
        )}`
      );
  ...
  const obtenerOpcionesPorID = (listaPlatos) => {
    return listaPlatos
      .map(
        (plato) => `\n ${plato.id}. ${plato.nombre} .. $${plato.precio.toFixed(2)}`
      )
      .join("");
  };
  ...
  do {
    mesa = parseInt(
      prompt("Ingrese numero de mesa para tomar el pedido").trim()
    );
